...
export const getFreelancerAction = createAsyncThunk(
  "user/Freelancer",
  async (id) => {
    const { data } = await axios.get(`users/${id}`);
    return data;
...
export const getAllSkillUserAction = createAsyncThunk(
  "user/skill",
  async (id) => {
...
export const statusUserAction = createAsyncThunk(
  "user/statusUser",
  async ({ idUser, status }) => {
    const { data } = await axios.get(
      `users/changestatus?idUser=${idUser}&status=${status}`
    );
    return data;
  }
);
